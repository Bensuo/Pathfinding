...
PathResult AStarPathfinder::FindPath(const Graph& graph, int start_index, int end_index) const
{
	...
	std::priority_queue<std::pair<int, std::shared_ptr<GraphNode>>, std::vector<std::pair<int, std::shared_ptr<GraphNode>>>, std::greater<std::pair<int, std::shared_ptr<GraphNode>>>> nodes;
